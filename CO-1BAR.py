...
tpot_data = pd.read_csv('/Users/goktug/Desktop/CO_TPOT_Onder/CO-1BAR.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('1 bar-CO (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['1 bar-CO (mol/kg)'], random_state=42)

# Average CV score on the training set was: -0.0011958793419648331
exported_pipeline = make_pipeline(
    StandardScaler(),
    GradientBoostingRegressor(alpha=0.99, learning_rate=0.1, loss="huber", max_depth=6, max_features=0.9500000000000001, min_samples_leaf=1, min_samples_split=6, n_estimators=100, subsample=0.45)
)
